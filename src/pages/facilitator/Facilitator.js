...
import { facCateg } from "../../actions/categories";
...
const Facilitator = () => {
  ...
  const test = async () => {
    if (sort) {
      await dispatch(facSort(sort));
      await dispatch(facSearch(search));
    } else {
      await dispatch(facSearchNoSort(search));
    }
  };
  ...
  useEffect(() => {
    if (sort) {
      dispatch(facSort(sort));
      setSearch("");
      setOrder(0);
    } else {
      dispatch(facConcerns());
      dispatch(facCateg());
      setOrder(0);
    }
  }, [dispatch, sort]);
                ...
                {concerns
                  ...
                  .map((concern, idx) => (
                    <TableRow
                      ...
                      key={idx}
                      onClick={() =>
                        history.push(`/Facilitator/concern/${concern._id}`)
                      }
                    >
