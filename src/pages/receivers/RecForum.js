...
import { recForum, recReply, recStatus } from "../../actions/concern";
...
const RecForum = () => {
  ...
  useEffect(() => {
    if (status) {
      dispatch(recStatus(id, status));
    } else if (id) {
      dispatch(recForum(id));
    }
  }, [dispatch, id, status]);
  return (
    <>
